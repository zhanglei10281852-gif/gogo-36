...
func TestDelegateGrantRejectsBroaderWildcardScope(t *testing.T) {
	registry, clock, _ := newTestRegistry(t)
	session, err := registry.CreateSession("agent-7", time.Hour, nil)
	if err != nil {
		t.Fatal(err)
	}
	parent, err := registry.IssueGrant(GrantRequest{
		SubjectID:      "agent-7",
		SessionID:      session.ID,
		Capabilities:   []string{"file.read"},
		ResourceScopes: []string{"repos/*/private/**"},
		TTL:            time.Hour,
		MaxUses:        10,
		Delegable:      true,
	})
	if err != nil {
		t.Fatalf("issue parent grant: %v", err)
	}
	_, err = registry.DelegateGrant(parent.ID, DelegationRequest{
		SubjectID:      "agent-7",
		SessionID:      session.ID,
		Capabilities:   []string{"file.read"},
		ResourceScopes: []string{"repos/**"},
		ExpiresAt:      clock.Now().Add(30 * time.Minute),
		MaxUses:        1,
	})
	if !errors.Is(err, ErrNotAuthorized) {
		t.Fatalf("broader wildcard scope delegation should fail: %v", err)
	}
}
